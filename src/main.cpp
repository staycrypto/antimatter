...
uint256 hashGenesisBlock("0xb8158727e8d4ca71f3053783a5edd573629d0ee70f6fc1a2a922eccec81e79cb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Antimatter: starting difficulty is 1 / 2^12
...
{
    // Antimatter: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    //nSubsidy >>= (nHeight / 840000); // Antimatter: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 60 ; // Diff retarget: Antimatter: 10 minutes
static const int64 nTargetSpacing = 5 * 60; // Antimatter: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Antimatter: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Antimatter: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Antimatter: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x809feaabfd0db26d63e643f78840752133c8a4f60bd4b49a49957f64b7aa9030");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ZeroHedge 09/Feb/2014 Is Googlecoin Coming?";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0.000001 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ee9eef0de63169c61b277f867eef2d28db7acbaa582a3286df4fe55d0a80b5238e321a64ce3c50804c5fecace75cd4f6243b0f58edb3a9d5f1452ec77c8cf45d") << OP_CHECKSIG;
        ...
        block.nVersion = 1;
        block.nTime    = 1391994985;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084641542;

        if (fTestNet)
        {
            block.nTime    = 1390637423;
            block.nNonce   = 386465977;
        }
	...
	if(fTestNet)
	{
		assert(block.hashMerkleRoot == uint256("0xe95eb0160d8d0aa8e428013d149d6fc36deca5f3bfe3fe7696a27c776b15223e"));
        }
        else
	{
	        assert(block.hashMerkleRoot == uint256("0xe95eb0160d8d0aa8e428013d149d6fc36deca5f3bfe3fe7696a27c776b15223e"));
...
unsigned char pchMessageStart[4] = { 0xc0, 0xfb, 0xb6, 0xdb }; // Antimatter: swap around.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AntimatterMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AntimatterMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AntimatterMiner(CWallet *pwallet)
{
    printf("AntimatterMiner started\n");
    ...
    try { loop {
        ...
        printf("Running AntimatterMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AntimatterMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AntimatterMiner, pwallet));
}
